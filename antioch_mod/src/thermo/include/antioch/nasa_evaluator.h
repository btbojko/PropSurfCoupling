...
public:
	...
	template<typename StateType>
	ANTIOCH_AUTO(StateType)
	h_i( const TempCache<StateType>& cache, unsigned int species ) const
	ANTIOCH_AUTOFUNC(StateType,
	...
	>::type
	h_i( const TempCache<StateType>& cache, VectorStateType& h ) const;
...
protected:
	...
	//! store the heat of formation of the species
	std::vector<CoeffType> Hof;
...
: _nasa_mixture(cea_mixture), Hof(_nasa_mixture.chemical_mixture().n_species())
  {
	return;
  }
...
template<typename CoeffType, typename NASAFit>
inline
void NASAEvaluator<CoeffType,NASAFit>::initHof()
{
	const Antioch::TempCache<CoeffType> tc(298.);
	for(unsigned int s = 0; s<Hof.size(); s++){
		Hof[s] = this->chem_mixture().R(s)*tc.T*this->h_over_RT(tc,s);
		std::cout << "Heat of formation, Hof[" << s << "] = " << Hof[s] << std::endl;
	}
	return;
}
template<typename CoeffType, typename NASAFit>
...
has_size<VectorStateType>::value, void
>::type
NASAEvaluator<CoeffType,NASAFit>::h_i( const TempCache<StateType>& cache, VectorStateType& h_i ) const
{
	antioch_assert_equal_to( h_i.size(), this->n_species() );

	for( unsigned int s = 0; s < this->n_species(); s++ )
	{
		h_i[s] = this->chem_mixture().R(s)*cache.T*this->h_over_RT(cache,s) - Hof[s];
	}
