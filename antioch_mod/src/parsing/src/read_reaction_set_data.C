		...
		ParsingType type )
{
	...
	while (parser->reaction())
	{
		...
		//        double reacNum = std::atof((parser->reaction_id()).c_str());
		//        if(reacNum==147){
		//        	int pauseHere{1};
		//        }
		...
		{
			if(parser->verify_Kooij_in_place_of_Arrhenius() && verbose)
			{
		...
		while(parser->rate_constant(reading_kinetics_model)) //for duplicate and falloff models, several kinetics rate to load, no mixing allowed
		{
			...
			{
				is_falloff = verbose;
				//TODO k0 should be second to be in line with the xml format, or should be explicitly stated
