...
namespace Antioch
{
    ...
    _Troe(NULL)
  {
    ...
    _default_unit[ParsingKey::PREEXP]                = "cm3/mol"; // originally "m3/kmol" which isn't really standard
            ...
            while(nasa)
              {
                ...
                split_string(std::string(coeffs->GetText())," ",tmp1);
                for(int i=0; i<tmp1.size(); i++){
                	std::vector<std::string> tmp;
                	split_string(tmp1[i],",",tmp);
                	for(int j=0; j<tmp.size();j++){
                		coeffs_str.push_back(tmp[j]);
                	}
                }
